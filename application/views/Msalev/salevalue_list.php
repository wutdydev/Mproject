                        ...
                        <td align='left'>
                            ...
                            <?php
                                if ($this->session->userdata('type') != 5) {
                                    ?>
                                    <button type="button" class="btn btn-outline btn-success btn-sm" data-placement="top" title="ใบปะหน้าวางบิล" onClick="window.open('<?php echo base_url('Salev/Maindata/INVOICE') . '/' . $res->tb1_data_id ?>')">&nbsp;<i class="fa fa-file-pdf-o" ></i>&nbsp;</button>
                                    <button type="button" class="btn btn-outline btn-primary btn-sm" data-placement="top" title="ใบส่งของ" onClick="window.open('<?php echo base_url('Salev/MINVOICE/INS') . '/' . $res->tb1_data_id ?>')">&nbsp;<i class="fa fa-ambulance" ></i>&nbsp;</button> 
                                    <?php
                                }
                                ?>
                                <?php
                                if ($res->tb1_md_approved == 1 and in_array($this->session->userdata('type'), array(5, 1, 7))) {
                                    ?>
                                    <button type="button" class="btn btn-outline btn-primary btn-sm" data-placement="top" title="ออกใบกำกับภาษี/ใบวางบิล"  onclick="window.open('<?php echo base_url('Salev/Maindata/BV') . '/' . $res->tb1_data_id ?>')">&nbsp;<span class="glyphicon glyphicon-export" aria-hidden="true"></span> B/V</button>
                                    <?php
                                }
                                ?>
                                ...
                                if ($this->session->userdata('type') == 1) {
                                    ?>
                                    <button type="button" class="btn btn-outline btn-danger btn-sm confirmation" href="<?php echo base_url('Salev/Maindata/Delete') . '/' . $res->tb1_data_id ?>">&nbsp;<i class="fa fa-trash-o" ></i>&nbsp;</button>
                                    <?php
                                }
                                ?>
                        </td>
